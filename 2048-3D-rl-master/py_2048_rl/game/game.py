...
ACTION_NAMES = ["left", "up", "right", "down", "front", "back"]
ACTION_LEFT = 0
ACTION_UP = 1
ACTION_RIGHT = 2
ACTION_DOWN = 3
...
class Game(object):
  ...
  def __init__(self, state=None, initial_score=0):
    ...
    if state is None:
      self._state = np.zeros((3, 3, 3), dtype=np.int)
  ...
  def available_actions(self):
    ...
    return [action for action in range(6) if self.is_action_available(action)]

  def is_action_available(self, action):
    """Determines whether action is available.
    That is, executing it would change the state.
    """
    if action < 4: # If action is not front or back
      temp_state = np.rot90(self._state, action, axes = (1,2))
    if action == 4: # If action is front
      temp_state = np.rot90(self._state, 3, axes = (0,2))
    if action == 5: # If action is back
      temp_state = np.rot90(self._state, axes = (0,2))
  ...
  def _is_action_available_left(self, state):
    ...
    for layer in range(3):
      for row in range(3):
        has_empty = False
        for col in range(3):
          has_empty |= state[layer, row, col] == 0
          if state[layer, row, col] != 0 and has_empty:
            return True
          if (state[layer, row, col] != 0 and col > 0 and
            state[layer, row, col] == state[layer, row, col - 1]):
            return True
  ...
  def do_action(self, action):
    ...
    if action < 4:
      self._state = np.rot90(temp_state, -action, axes = (1,2))
    if action == 4:
      self._state = np.rot90(temp_state, axes = (0,2))
    if action == 5:
      self._state = np.rot90(temp_state, 3, axes = (0,2))
    self._score += reward
  ...
  def _do_action_left(self, state):
    ...
    for layer in range(3):
      for row in range(3):
        ...
        merge_candidate = -1
        merged = np.zeros((3,), dtype=np.bool)

        for col in range(3):
          if state[layer, row, col] == 0:
            continue

          if (merge_candidate != -1 and
            not merged[merge_candidate] and
            state[layer, row, merge_candidate] == state[layer, row, col]):
          			# Merge tile with merge_candidate
            state[layer, row, col] = 0
            merged[merge_candidate] = True
            state[layer, row, merge_candidate] += 1
            reward += 2 ** state[layer, row, merge_candidate]
  ...
  def add_random_tile(self):
    ...
    x_pos, y_pos, z_pos = np.where(self._state == 0)
    assert len(x_pos) != 0
    empty_index = np.random.choice(len(x_pos))
    value = np.random.choice([2, 4], p=[0.8, 0.2])

    self._state[x_pos[empty_index], y_pos[empty_index], z_pos[empty_index]] = value
  ...
  def print_state(self):
    ...
    for layer in range(3):
      for row in range(3):
        print ("|" + "|".join([tile_string(v) for v in self._state[layer, row, :]]) + "|")
        print ("-" * 25)
